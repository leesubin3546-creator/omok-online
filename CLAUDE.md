...
**티카투카 배치 모션**: ①클릭 칸 ≠ 실제 배치 칸(안쪽 압축 정렬)일 때 **고스트 주사위가 클릭 위치→최종 위치로 날아감** (`ttpFlyGhost`, `TTP.lastClick` 기록 후 렌더 diff에서 소비, position:fixed+transition 0.28s). ②압축 정렬로 값이 밀린 칸은 `.ttp-shift` 애니메이션 (pv>0&&nv>0&&pv!==nv).

**관전/종료 팝업 UX**:
- game-screen에 `#spectator-actions`(👁 관전 나가기 → goLobby) — 관전 시 표시 (기존엔 나가기 버튼이 없었음)
- 결과 오버레이(오목·티카투카)에 **"💬 닫고 채팅하기"** — 닫으면 `#postgame-bar`/`#ttp-postgame-bar` 표시: [🔄 재대결][로비로][📊 결과 다시보기]. 관전자에겐 재대결 숨김. 게임 시작/나가기 시 바 정리. 관전자는 팝업 닫고 계속 관전 → 재대결 시작되면 자동으로 새 게임 스트림.

검증: 올인빵 매칭 7케이스(200만vs3천 → 3천 매칭/승자 6천/원금보존/무승부 반환) + UX 10케이스(오버레이 닫기/관전자 분기/고스트 팝 위치/shift 표시) 통과.
